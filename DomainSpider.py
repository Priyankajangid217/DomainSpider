...
def Search(query, queryFile1, queryFile2):
    ...
    for link in search(query, tld="co.in", start=startedFrom, num=searchResults, stop=searchResults, pause=5):
        links.append(link)
    print('\nUnique Domain Name(s) for the search term '+query+':')

    if len(links)==0:
        print('No Links Found!')
        print('\nDomainSpider wrote '+str(finalCount)+' links in the current session')
    ...
    print('DomainSpider wrote '+str(uniqLenLink)+' link(s) with a total of '+str(endedAt)+' unique Domain name(s) for the search term "'+query+'".')
    print('Unique Domain Link(s) for the search term "'+query+'" stored in "'+queryFile2+'" and cache file stored as "'+queryFile1+'"')
    print('DomainSpider wrote '+str(finalCount)+' link(s) in the current session.')
...
print('Welcome to DomainSpider!\nPress Enter to Search / Press H for help')
